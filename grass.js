...
async function connectWebSocket(userId) {
  ...
  ws.on("message", data => {
    ...
    if (message.action === "AUTH") {
      ...
      ws.send(JSON.stringify({ id: message.id, origin_action: "PONG" }));
    }
  });
...
async function connectWebSocketWithProxy(userId, proxy) {
  ...
  ws.on("open", () => {
    console.log(`\x1b[34m[${userId}] Koneksi berhasil dengan proxy: ${proxy}\x1b[0m`);
    const validateJwt = JSON.stringify({
      jwt: { action: "getCurrentVersion" },
      action: "VALIDATE_JWT",
    });
  ...
  ws.on("error", error => {
    console.error(`\x1b[31m[${userId}] Terjadi kesalahan dengan proxy ${proxy}: ${error.message}\x1b[0m`);
  });

  ws.on("close", () => {
    console.log(`\x1b[31m[${userId}] Koneksi ditutup dengan proxy ${proxy}\x1b[0m`);
    setTimeout(() => connectWebSocketWithProxy(userId, proxy), 5000);
  });
}
  ...
  rl.question("\x1b[33mPilih opsi (1-4):\x1b[0m ", async option => {
    if (option === "1") {
      ...
      if (userIds.length > 0) {
        ...
        console.log("\x1b[31mTidak ada User ID yang ditemukan!\x1b[0m");
        console.log("===================");
        mainMenu();
      }
    } else if (option === "3") {
      ...
      if (userIds.length > 0 && proxies.length > 0) {
        for (const userId of userIds) {
          for (const proxy of proxies) {
            connectWebSocketWithProxy(userId, proxy);
            await delay(1000);
          }
        }
      } else {
        console.log("\x1b[31mTidak ada User ID atau proxy yang ditemukan!\x1b[0m");
        console.log("===================");
        mainMenu();
      }
    } else if (option === "4") {
      console.log("\x1b[32mTerima kasih telah menggunakan program ini!\x1b[0m");
      rl.close();
      process.exit(0);
    } else {
      console.log("\x1b[31mOpsi tidak valid!\x1b[0m");
      console.log("===================");
      mainMenu();
    }
  });
